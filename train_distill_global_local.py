...
from data.brain_distill import get_loaders
...
class Solver:
    def __init__(self, data_files, opt):
        ...
        self.g_conv_dim = self.opt.g_conv_dim
        self.g_repeat_num = self.opt.g_repeat_num
        ...
        self.lambda_feature = self.opt.lambda_feature
        self.lambda_sup = self.opt.lambda_sup
    ...
    def build_model(self):
        self.teacher = UNet_middle_concat(used_modality_num=1)
        self.teacher.to(self.device)
        Teacher_path = 'checkpoint_teacher/model_save_dir/100-G.ckpt'
        self.teacher.load_state_dict(torch.load(
                Teacher_path, map_location=lambda storage, loc: storage))

        self.G = UNet_middle_concat(used_modality_num=1)
        self.G.to(self.device)
        self.G.load_state_dict(torch.load(Teacher_path, map_location=lambda storage, loc: storage))
        init_weights(self.G.tumor_encoder, init_type='kaiming', init_gain=0.02)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for i, (images, labels, vec_org, label_org, vec_trg, label_trg) in enumerate(loader):

                cur_step = epoch * len(loader) + i

                image = images[0].to(self.device)            
                
                labels[0] = labels[0].unsqueeze(1).float().to(self.device)
                x_local_gather = self.norm(labels[0] * self.denorm(image))
                ...
                x_fake, x_local_target_gather, _ = self.G(image, image, vec_trg)
                ...
                if (i + 1) % self.n_critic == 0:
                    x_fake, x_local_target_gather, feature_S = self.G(image, image, vec_trg)
                    ...
                    x_sup, feature_T = self.teacher.forward_teacher(image, x_local_gather, vec_trg)    # 教师模型

                    g_loss_sup = torch.mean(torch.abs(x_fake - x_sup))
                    g_loss_feature = torch.mean(torch.abs(feature_S - feature_T))

                    x_target_local_gather = self.norm(labels[0] * self.denorm(x_fake))
                    
                    x_rec, x_local_rec_gather, _  = self.G(x_fake, x_fake, vec_org)
                    ...
                    g_loss = self.lambda_fake * (g_loss_fake + g_loss_fake_local) + self.lambda_cls * \
                        (g_loss_cls + g_loss_cls_local) + self.lambda_rec * g_loss_rec +  \
                            self.lambda_local * g_loss_local + self.lambda_sup * g_loss_sup + \
                                self.lambda_feature * g_loss_feature
                    ...
                    loss['G/g_loss_sup'] = g_loss_sup.item()
                    loss['G/g_loss_feature'] = g_loss_feature.item()
    ...
    def val(self, epoch):
        ...
        num = 0
        with torch.no_grad():
            for i, (images, vec, cls, name, GT_img) in enumerate(loader):               
                x = images[0].to(self.device)
                ...
                for c in range(self.c_dim):
                    ...
                    x_fake, x_local_target_gather, _ = self.G(x, x, fake_vec)
    ...
    def infer(self, epoch):
        ...
        with torch.no_grad():
            for i, (images, vec, cls, names) in enumerate(self.loaders['test']):
                x = images[0].to(self.device)
                vec = vec.to(self.device)
                ...
                for c in range(self.c_dim):
                    ...
                    fake_vec = self.label2onenot(
                        fake_cls, self.c_dim).to(self.device)
                    x_fake, _, _ = self.G(x, x, fake_vec)
...
if __name__ == '__main__':
    ...
    args.add_argument('--lambda_sup', type=float, default=10)
    args.add_argument('--lambda_feature', type=float, default=10)
